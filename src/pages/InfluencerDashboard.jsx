...
import { registerInfluencer, enrollProject, getProjectSearch, getInfluencerActiveProjects } from '../api/api';
...
export default function InfluencerDashboard() {
  ...
  const [formData, setFormData] = useState({
    fullName: '', email: '', phone: '', country: 'United States', bio: '',
    causes: [],
    snsAccounts: [],
    idType: 'Aadhaar Card', idNumber: '',
    accountHolder: '', bankName: 'Chase Bank', accountNumber: '', routingNumber: '', accountType: 'Checking',
  });
  ...
  const handleEnroll = async () => {
    ...
    setEnrolling(true);
    try {
      await enrollProject(selectedProject.id, influencerId);
      setEnrollSuccess(true);
      fetchActiveProjects(influencerId);
      setTimeout(() => { setEnrollSuccess(false); nav('proj-active'); }, 1500);
    } catch (e) {
      console.error(e);
    }
    setEnrolling(false);
      ...
      <div className="layout" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <aside className="sidebar">
          ...
          <div className="sidebar-user">
            ...
            <div style={{ flex: 1 }}>
              <div className="user-name">Jamie Rivera</div>
              <div className="user-handle">@jamie.forchange</div>
            </div>
          </div>
        ...
        <div className="main">
          <header className="topbar">
            ...
            <div className="topbar-actions">
              ...
              <div className="user-avatar" style={{ cursor: 'pointer' }} onClick={() => nav('reg-profile')}>JR</div>
            </div>
          </header>
          ...
          <div className="page-content fade-up" key={page}>
                  ...
                  <div className="card">
                    ...
                    <div className="form-group">
                      ...
                      <select className="form-select" value={formData.country} onChange={e => setFormData(f => ({ ...f, country: e.target.value }))}>
                        {['United States','United Kingdom','Canada','India','Germany'].map(c => <option key={c}>{c}</option>)}
                      </select>
                    </div>
                  </div>
                  <div>
              ...
              <>
                ...
                <div className="grid-2">
                  ...
                  <div className="card">
                    ...
                    <div className="card-sub">Link at least 2 accounts to apply for campaigns</div>
                    <div className="grid-2" style={{ marginBottom: 16 }}>
                      {[
                        { val: `${formData.snsAccounts.length}`, lbl: 'Accounts Linked', color: 'var(--navy)' },
                        { val: formData.snsAccounts.length >= 2 ? '✓ Ready' : '⚠ Need 2+', lbl: 'Campaign Eligibility', color: formData.snsAccounts.length >= 2 ? 'var(--green-mid)' : 'var(--orange-mid)' },
                      ].map(k => (
            ...
            {page === 'reg-kyc' && (
              <>
                ...
                <div className="grid-2">
                  <div className="card">
                    <div className="card-title">Document Submission</div>
                    <div className="card-sub">Upload clear, valid government-issued photo ID</div>
                    <div className="form-group">
                      <label className="form-label">ID Type</label>
                      <select className="form-select" value={formData.idType} onChange={e => setFormData(f => ({ ...f, idType: e.target.value }))}>
                        {['Aadhaar Card','PAN Card','Passport','Driver\'s Licence'].map(t => <option key={t}>{t}</option>)}
                      </select>
                    </div>
                    <div className="form-group">
                    ...
                    <div className="upload-box" style={{ marginBottom: 12 }}>
                      ...
                      <div className="upload-text"><strong>Click to upload</strong> front of ID or drag &amp; drop</div>
                      <div style={{ fontSize: 11, color: 'var(--slate)', marginTop: 4 }}>JPG, PNG — max 5MB</div>
                    </div>
                    <div className="upload-box">
                      <div className="upload-icon">🪪</div>
                      <div className="upload-text"><strong>Click to upload</strong> back of ID or drag &amp; drop</div>
                      <div style={{ fontSize: 11, color: 'var(--slate)', marginTop: 4 }}>JPG, PNG — max 5MB</div>
                    </div>
                  </div>
                  <div>
                    <div className="card" style={{ marginBottom: 16 }}>
                      ...
                      <div className="card-sub">A selfie holding your ID for liveness check</div>
                      <div className="upload-box">
                        <div className="upload-icon">🤳</div>
                        <div className="upload-text"><strong>Click to upload</strong> selfie with ID</div>
                        <div style={{ fontSize: 11, color: 'var(--slate)', marginTop: 4 }}>JPG, PNG — max 5MB</div>
                      </div>
                    </div>
                    <div className="card">
                      ...
                      <div className="timeline">
                        {[
                          { title: 'Documents Submitted', meta: 'Pending submission', done: false },
                          { title: 'Auto-verification', meta: 'Pending', done: false },
                          { title: 'Manual Review', meta: 'Pending', done: false },
                          { title: 'KYC Approval', meta: 'Pending', done: false },
                        ].map((t, i) => (
                ...
                <div className="grid-2">
                  <div className="card">
                    ...
                    {[
                      ...
                      { label: 'Routing Number *', key: 'routingNumber', placeholder: '9-digit routing number' },
                    ].map(({ label, key, placeholder }) => (
                      <div key={key} className="form-group">
                        <label className="form-label">{label}</label>
                        <input className="form-input" value={formData[key]} placeholder={placeholder}
                    ...
                    <div className="form-group">
                      ...
                      <select className="form-select" value={formData.bankName} onChange={e => setFormData(f => ({ ...f, bankName: e.target.value }))}>
                        {['Chase Bank','Bank of America','Wells Fargo','Citibank','Other'].map(b => <option key={b}>{b}</option>)}
                      </select>
                    </div>
                    <div className="form-group">
              ...
              <>
                ...
                <div className="grid-2" style={{ marginBottom: 20 }}>
                  {projects.map((p, i) => {
                    ...
                    return (
                      <div key={p.id} className="proj-card" onClick={() => { setSelectedProject(p); nav('proj-apply'); }}>
                        ...
                        <div className="proj-card-body">
                          ...
                          <div className="proj-card-footer">
                            ...
                            {alreadyEnrolled
                              ...
                              : <button className="btn btn-primary btn-sm" onClick={e => { e.stopPropagation(); setSelectedProject(p); nav('proj-apply'); }}>Apply →</button>
                            }
                          </div>
                        </div>
                      </div>
            ...
            {page === 'proj-apply' && (
              <>
                ...
                {enrollSuccess && (
                  <div className="alert-banner" style={{ background:'var(--green-light)', color:'var(--green)', borderColor:'var(--green-mid)' }}>✅ Successfully enrolled! Redirecting to your active projects…</div>
                )}
                {selectedProject ? (
